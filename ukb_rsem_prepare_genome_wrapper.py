...
def steward(config_file_path:str, ukb_rsem_prepare_genome_path:str, send_message_script:str) -> None:
    ...
    while True:
        with open(execution_trace_file, encoding = 'UTF-8') as trace_f:
            for line in trace_f:
                if 'RSEM_PREPAREREFERENCE' in line:
                    if 'COMPLETED' in line:
                        feedback_dict['endDate'] = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())
                        feedback_dict['error'] = 0
                        for file in os.listdir('{}/results'.format(analysis_path)):
                            if file.endswith('transcripts.fa'):
                                transcripts_fa = '{}/results/{}'.format(analysis_path, file)
                            if file.endswith('tar.gz'):
                                tar_gz = '{}/results/{}'.format(analysis_path, file)
                        report_download_dict = {
                            ...
                            'text': [
                                {
                                    'sort':1,
                                    'title': 'RSEM构建基因组结果文件1',
                                    'content': 'RSEM构建基因组结果文件1：#&{}'.format(transcripts_fa), 
                                    'postDes': '', 
                                    'memo': '', 
                                    'preDes': ''
                                },
                                {
                                    'sort':1,
                                    'title': 'RSEM构建基因组结果文件2',
                                    'content': 'RSEM构建基因组结果文件2：#&{}'.format(tar_gz), 
                                    'postDes': '', 
                                    'memo': '', 
                                    'preDes': ''
                                }
...
def main() -> None:
    ...
    parser.add_argument('--ukb_rsem_prepare_genome_path', required=True, help='the full path of the ukb_rsem_prepare_genome')
